...
-- 
-- IMPORTANTE: Execute este script APÓS fazer login no sistema
-- O script usará automaticamente o primeiro usuário autenticado
-- =====================================================

DO $$
DECLARE
  v_partner_id uuid;
  v_client_id_1 uuid := gen_random_uuid();
  v_client_id_2 uuid := gen_random_uuid();
  v_client_id_3 uuid := gen_random_uuid();
  v_vehicle_id_1 uuid := gen_random_uuid();
  v_vehicle_id_2 uuid := gen_random_uuid();
  v_vehicle_id_3 uuid := gen_random_uuid();
BEGIN
  -- Obter o primeiro usuário autenticado do sistema
  SELECT id INTO v_partner_id 
  FROM auth.users 
  WHERE email IS NOT NULL 
  LIMIT 1;
  
  -- Se não houver usuário, abortar com mensagem clara
  IF v_partner_id IS NULL THEN
    RAISE EXCEPTION 'ERRO: Nenhum usuário encontrado. Por favor, faça login no sistema primeiro e execute este script novamente.';
  END IF;

  RAISE NOTICE 'Usando partner_id: %', v_partner_id;

  -- =====================================================
  -- CRIAR CLIENTES DE TESTE
  -- =====================================================

  INSERT INTO public.clients (id, partner_id, name, email, phone, cpf_cnpj, created_at, updated_at)
  VALUES
    (
      v_client_id_1,
      v_partner_id,
  ...
  RAISE NOTICE 'Clientes criados com sucesso!';
  ...
  RAISE NOTICE 'Veículos criados com sucesso!';

  -- =====================================================
  -- VINCULAR VEÍCULOS À FROTA DO PARCEIRO
  -- =====================================================
  ...
  RAISE NOTICE 'Frota vinculada com sucesso!';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'SEED EXECUTADO COM SUCESSO!';
  RAISE NOTICE '3 clientes criados';
  RAISE NOTICE '3 veículos criados (Honda Civic, Toyota Corolla, VW Gol)';
  RAISE NOTICE '3 vínculos na frota criados';
  RAISE NOTICE '========================================';
