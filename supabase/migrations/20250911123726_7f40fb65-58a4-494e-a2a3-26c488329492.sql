-- Corrigir problemas de segurança - versão final corrigida

-- 1. Habilitar RLS nas tabelas restantes
...
-- 2. Criar políticas básicas (usar FOR ALL onde possível para simplificar)
...
-- Review metrics: ler todos, admin modifica
CREATE POLICY "all_review_metrics_read" ON public.review_metrics
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "admin_review_metrics_modify" ON public.review_metrics
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.users 
      WHERE id::text = auth.uid()::text 
...
CREATE POLICY "users_own_service_reviews" ON public.service_reviews
  FOR ALL USING (user_id::text = auth.uid()::text);

-- Service categories: ler todos, admin modifica
CREATE POLICY "all_service_categories_read" ON public.service_categories
  FOR SELECT USING (auth.role() = 'authenticated');

CREATE POLICY "admin_service_categories_modify" ON public.service_categories
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.users 
      WHERE id::text = auth.uid()::text 
